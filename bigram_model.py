...
encode = lambda s: [string_to_integers[c] for c in s]   # encoder : take a string, output a list of integers
decode = lambda l: ''.join([integers_to_string[i] for i in l])   # decoder : take a list of integers, output a string
...
class BigramLanguageModel(nn.Module):
  ...
  def forward(self, idx, targets=None):
    ...
    tok_emb = self.token_embedding_table(idx)   # (B,T,C) - Batch (4) by Time (8) by Channel (vocab_size, i.e, 65) tensor
